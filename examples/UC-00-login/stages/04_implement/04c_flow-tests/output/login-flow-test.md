...
- **Expected token chain (parent → child):**
  1. `Web.request { requestId }`
  2. `UserNaming.lookupByUsername { username: "ada", userId: U, outcome: FOUND }`
...
- **Expected token chain:**
  1. `Web.request`
  2. `UserNaming.lookupByUsername { outcome: FOUND }`
...
- **Expected token chain:**
  1. `Web.request`
  2. `UserNaming.lookupByUsername { outcome: NOT_FOUND }`
