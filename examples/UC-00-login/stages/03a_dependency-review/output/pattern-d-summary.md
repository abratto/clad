...
- Pattern B (a flow-sibling output — e.g. `?user` from
  `UserNaming/lookupByUsername` consumed by `CheckForLoginWhenLookupByUsernameFound`, or
  `?sid` from `Session/grant` consumed by `RespondForLoginWhenGrantGranted` in
  the same flow), or
