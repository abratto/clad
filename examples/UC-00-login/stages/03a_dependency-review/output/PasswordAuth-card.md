...
|---|---|---|---|---|
| `check` | `CheckForLoginWhenLookupByUsernameFound` (`successful-login`, `wrong-password`, `lockout`) | `userId`, `password` | A + B | `?p` from `Web/request` trigger (A); `?user` from `UserNaming/lookupByUsername` completion (B) |

> `PasswordAuth/check`'s lockout branch is expressed by the
> `Locked` outcome plus `RespondForLoginWhenCheckLocked`, not by a separate `lock`
> action.
...
- The sync `CheckForLoginWhenLookupByUsernameFound` exists under `../../03_syncs/output/`.
