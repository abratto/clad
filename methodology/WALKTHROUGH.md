...
- [`LoginGrantsSession.sync.md`](../features/UC-00-login/stages/03_syncs/output/LoginGrantsSession.sync.md) — `when PasswordAuth.check(userId, password) -> Ok` then `Session.grant(userId)` then `Web.respond(200, { sessionToken })`. The `where: B: sessionId = result_of(Session.grant).sessionId` is **Pattern B** (flow-sibling); see [`SYNC_PATTERNS.md`](architecture/SYNC_PATTERNS.md).
- [`LockoutOnFailedAttempts.sync.md`](../features/UC-00-login/stages/03_syncs/output/LockoutOnFailedAttempts.sync.md) — spec-only at this round.
...
- **04a (ORM):** the in-memory profile triggers `_NOT_APPLICABLE.md`. CONTEXT loaded: [`04a_orm/CONTEXT.md`](../features/UC-00-login/stages/04_implement/04a_orm/CONTEXT.md).
...
- **04c (Flow tests, outer red):** one `*-flow-test.md` per scenario + a `@Disabled` stub Java test under `reference-impl/…/flows/`. CONTEXT loaded: [`04c_flow-tests/CONTEXT.md`](../features/UC-00-login/stages/04_implement/04c_flow-tests/CONTEXT.md).
- **04d (Concept TDD, inner red→green):** one concept at a time, no cross-concept imports (R1), every action emits a flow token (R5). Builds [`concept-test-derivation.md`](../features/UC-00-login/stages/04_implement/04d_concept-tdd/output/concept-test-derivation.md). CONTEXT loaded: [`04d_concept-tdd/CONTEXT.md`](../features/UC-00-login/stages/04_implement/04d_concept-tdd/CONTEXT.md).
- **04e (Sync TDD, inner red→green; outer goes green):** TDD each sync, then re-enable the `04c` flow tests and watch them pass. Builds [`sync-test-derivation.md`](../features/UC-00-login/stages/04_implement/04e_sync-tdd/output/sync-test-derivation.md). CONTEXT loaded: [`04e_sync-tdd/CONTEXT.md`](../features/UC-00-login/stages/04_implement/04e_sync-tdd/CONTEXT.md).

**Gate questions:** one canonical gate after each sub-stage. The
final gate (after `04e`) is the gate before Stage 05.
