...
| See a worked example end-to-end | [`examples/UC-00-login/README.md`](examples/UC-00-login/README.md) (start at `stages/01_usecase/`; its Stage 00 outputs travel with the example at `examples/UC-00-login/system-stage-00/`) |
...
| Give each stage its own context (sub-agents or fresh sessions) | Open [`methodology/implementation/STAGES.md`](methodology/implementation/STAGES.md) §"Orchestration: one sub-agent per stage" — the recommended loop when your harness can spawn sub-agents, plus the `workflow.session-per-stage` fallback. |
...
| Reuse or propose a concept | Once the project has a vocabulary: `features/_system/concepts-catalog.md` (generated index), `features/_system/concept-dependence.md` (reviewed graph), and `features/_system/concepts/` (the corpus). Promote approved proposals with `./clad promote-concepts`. A project's corpus starts **empty** — it is not inherited from the worked example. |
| Author a new concept / sync / use case / actors / goals / spec | [`templates/`](templates/) |
| See the optional Java reference impl | [`reference-impl/README.md`](reference-impl/README.md) — canonical: `java-legible` on the fire-after-commit engine |
