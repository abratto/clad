...
The corpus starts **empty** — an app's domain is not known in advance, so CLAD
seeds no concepts. Proposed concepts are promoted into it only on gate
approval, via `./clad promote-concepts` — never by hand.
`concepts-catalog.md` is regenerated by
`quality-gate/generate_concepts_catalog.py`; edit the corpus, not the index.
The completed example's vocabulary
(`examples/UC-00-login/stages/02_concepts/output/`) is frozen history inside
the example, not part of this corpus.
...
- Worked single-goal example: [`../../examples/UC-00-login/`](../../examples/UC-00-login/) *(read the note there)*
