...
- `01b` also carries a derived consolidated `login-all-scenarios-chain.md`;
  current work produces one `<scenario>-chain.md` per scenario (the
  consolidated view is not canonical and is skipped by the generators).
- `03a` predates the `concept-matrix.md` output (added to the current
  contract, generated by `verify_concept_matrix.py`).
- `04c` preserves a historical native-track `login-flow-test.md` (and
  `_NOTE.md`); the canonical current output is the Gherkin `login.feature`
...
- `04d`/`04e` carry migration-placeholder derivation files whose table
  ...
  cannot parse them. New work must produce the current per-action
...
| 03 | [`stages/03_syncs/CONTEXT.md`](stages/03_syncs/CONTEXT.md) | [`LookupByUsernameForLoginWhenRequestRouted.sync.md`](stages/03_syncs/output/LookupByUsernameForLoginWhenRequestRouted.sync.md), [`CheckForLoginWhenLookupByUsernameFound.sync.md`](stages/03_syncs/output/CheckForLoginWhenLookupByUsernameFound.sync.md), [`RespondForLoginWhenLookupByUsernameRefused.sync.md`](stages/03_syncs/output/RespondForLoginWhenLookupByUsernameRefused.sync.md), [`GrantForLoginWhenCheckOk.sync.md`](stages/03_syncs/output/GrantForLoginWhenCheckOk.sync.md), [`RespondForLoginWhenCheckBadPassword.sync.md`](stages/03_syncs/output/RespondForLoginWhenCheckBadPassword.sync.md), [`RespondForLoginWhenCheckLocked.sync.md`](stages/03_syncs/output/RespondForLoginWhenCheckLocked.sync.md), [`RespondForLoginWhenGrantGranted.sync.md`](stages/03_syncs/output/RespondForLoginWhenGrantGranted.sync.md) | UC-00 now shows the canonical one-sync-per-transition shape. `RespondForLoginWhenGrantGranted` is the worked example's Pattern B join; the 401 branches are syncs too, so Stage 03 remains the single declarative coordination surface. |
...
| 04b | [`stages/04_implement/04b_contract/CONTEXT.md`](stages/04_implement/04b_contract/CONTEXT.md) | [`UserNaming.contract.md`](stages/04_implement/04b_contract/output/UserNaming.contract.md), [`PasswordAuth.contract.md`](stages/04_implement/04b_contract/output/PasswordAuth.contract.md), [`Session.contract.md`](stages/04_implement/04b_contract/output/Session.contract.md) | Mechanically-extracted Contract slices the test code compiles against. |
...
- [`../../methodology/architecture/SYNC_PATTERNS.md`](../../methodology/architecture/SYNC_PATTERNS.md) — the four legal `where` patterns; needed for `RespondForLoginWhenGrantGranted`'s Pattern B `where:` clause and for reading the 03a cards.
