...
| 03 | [`stages/03_syncs/CONTEXT.md`](stages/03_syncs/CONTEXT.md) | [`LookupUserForLogin.sync.md`](stages/03_syncs/output/LookupUserForLogin.sync.md), [`CheckCredentialForLogin.sync.md`](stages/03_syncs/output/CheckCredentialForLogin.sync.md), [`RespondUnknownUser.sync.md`](stages/03_syncs/output/RespondUnknownUser.sync.md), [`GrantSessionForLogin.sync.md`](stages/03_syncs/output/GrantSessionForLogin.sync.md), [`RespondWrongPassword.sync.md`](stages/03_syncs/output/RespondWrongPassword.sync.md), [`RespondLocked.sync.md`](stages/03_syncs/output/RespondLocked.sync.md), [`RespondLoginSuccess.sync.md`](stages/03_syncs/output/RespondLoginSuccess.sync.md) | UC-00 now shows the canonical one-sync-per-transition shape. `RespondLoginSuccess` is the worked example's Pattern B join; the 401 branches are syncs too, so Stage 03 remains the single declarative coordination surface. |
...
| 04d-red | [`stages/04_implement/04d_concept-tdd/04d_red-tests/CONTEXT.md`](stages/04_implement/04d_concept-tdd/04d_red-tests/CONTEXT.md) | concept test derivation artefact (not yet committed) + per-concept test files | Derive and approve red concept tests from `04c` + `04b`; no implementation code. |
...
| 04e-red | [`stages/04_implement/04e_sync-tdd/04e_red-tests/CONTEXT.md`](stages/04_implement/04e_sync-tdd/04e_red-tests/CONTEXT.md) | sync test derivation artefact (not yet committed) + per-sync test files | Derive and approve red sync tests from Stage 03 + `04c`; no implementation code. |
...
- [`../../methodology/architecture/SYNC_PATTERNS.md`](../../methodology/architecture/SYNC_PATTERNS.md) — the four legal `where` patterns; needed for `RespondLoginSuccess`'s Pattern B `where:` clause and for reading the 03a cards.
